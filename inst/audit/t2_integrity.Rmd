---
title: "TP2 Data Integrity Check"
author: "Tristan Mahr"
date: "`r Sys.Date()`"
...
```{r}
reentry_participants <- df_tp2_redo %>%
  select(Source, ParticipantID) %>%
  distinct

original_participants <- df_t2_info_long %>%
  select(Source, ParticipantID) %>%
  distinct
```
...
both <- bind_rows(df_t2_info_long, df_tp2_redo) %>%
