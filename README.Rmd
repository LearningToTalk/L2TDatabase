...
into analysis scripts. This package provides `make_cnf_file` which creates a
cnf file from login and connection information.
...
Once we've created a .cnf file, we can connect to the database using 
...
As I've worked on the back-end of the database, I've been using the Comment
fields to describe the data that goes into each field. We can download these
comments along with other pieces of information about a table by using
`describe_tbl`. With this function, we can quickly create a "codebook" to
accompany our data. I have [blogged about][blog-backup] the implementation of 
these metadata-related functions.
...
These two forms of metadata are backed up by `l2t_backup` as well. They are 
stored in a `metadata` folder.
...
statements that can be used to reconstruct the database.
...
developed very conservative helper functions for writing data. In fact there is 
only such function so far: `append_rows_to_table`. (I'd like to add an
